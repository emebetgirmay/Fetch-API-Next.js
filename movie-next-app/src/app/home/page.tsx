            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {searchResults.map((item) => (
                <div key={item.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-transform">
                  ...
                  />
                  <div className="p-6">
                    <h3 className="text-lg font-semibold text-pink-300 truncate">{item.title}</h3>
          ...
          <>
                ...
                >
                  <div className="bg-gradient-to-r from-gray-950/80 to-transparent h-full flex flex-col justify-center px-8 lg:px-16">
                    ...
                    <div className="flex gap-6">
                      <button className="bg-white text-gray-900 px-8 py-3 rounded-lg font-semibold hover:bg-pink-400 transition-colors">
                        Watch Now
                      </button>
                      <button
                        onClick={() => toggleFavorite(movie)}
                        className={`px-8 py-3 rounded-lg font-semibold border border-pink-300 ${
                          favoriteList.some((fav) => fav.id === movie.id)
                            ? 'bg-white text-gray-900'
                            : 'text-white'
                        } hover:bg-pink-400 hover:text-gray-900 transition-colors`}
                      >
            ...
            <section>
              <button
                ...
                className="mb-8 bg-pink-300 text-gray-900 px-6 py-3 rounded-lg font-semibold hover:bg-pink-400 transition-colors"
              >
              ...
              {displayFavorites ? (
                <div>
                  <h2 className="text-3xl font-bold text-pink-300 mb-8">Your Favorites</h2>
                  {favoriteList.length === 0 ? (
                    <p className="text-pink-300 text-lg">No favorites added yet.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                      {favoriteList.map((item) => (
                        <div key={item.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-transform hover:scale-105">
                          ...
                          <div className="p-6">
                            <h3 className="text-lg font-semibold text-pink-300 truncate">{item.title}</h3>
                            <p className="text-pink-300 text-sm mt-1">{new Date(item.release_date).getFullYear()}</p>
                            <button
                              onClick={() => toggleFavorite(item)}
                              className="mt-4 bg-pink-300 text-gray-900 px-4 py-2 rounded-lg font-medium hover:bg-pink-400 transition-colors"
                            >
                              Remove Favorite
                            </button>
                          </div>
                ...
                <>
                  <h2 className="text-3xl font-bold text-pink-300 mb-8">Latest Movies</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-16">
                    {sortedMovies.map((movie) => (
                      <div key={movie.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-transform hover:scale-105">
                        ...
                        />
                        <div className="p-6">
                          <h3 className="text-lg font-semibold text-pink-300 truncate">{movie.title}</h3>
                          <p className="text-pink-300 text-sm mt-1">{new Date(movie.release_date).getFullYear()}</p>
                          <button
                            onClick={() => toggleFavorite(movie)}
                            className={`mt-4 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                              favoriteList.some((fav) => fav.id === movie.id)
                                ? 'bg-pink-300 text-gray-900'
                                : 'border border-pink-300 text-pink-300'
                            } hover:bg-pink-400 hover:text-gray-900`}
                          >
                            {favoriteList.some((fav) => fav.id === movie.id) ? 'Remove Favorite' : 'Add to Favorites'}
                          </button>
                        </div>
                  ...
                  <h2 className="text-3xl font-bold text-pink-300 mb-8">Latest Series</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    {sortedSeries.map((series) => (
                      <div key={series.id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-transform hover:scale-105">
                        ...
                        />
                        <div className="p-6">
                          <h3 className="text-lg font-semibold text-pink-300 truncate">{series.title}</h3>
                          <p className="text-pink-300 text-sm mt-1">{new Date(series.release_date).getFullYear()}</p>
                          <button
